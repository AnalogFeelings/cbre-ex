...
namespace CBRE.Rendering
{
    public static class MathFunctions
    {
        ...
        {
            Vector4 source = new Vector4(coordinate.X, coordinate.Y, coordinate.Z, 1);
            Vector4 imed = Vector4.Transform(source, modelview);
            Vector4 vector = Vector4.Transform(imed, projection);
            if (vector.W < 0.00001) return new Vector3(1000000, 1000000, 1000000);
            Vector3 result = Vector3.Divide(new Vector3(vector.X, vector.Y, vector.Z), vector.W);
        ...
        {
            if (!Matrix4x4.Invert(Matrix4x4.Multiply(modelview, projection), out Matrix4x4 matrix)) throw new Exception("Matrix cannot be inverted.");
